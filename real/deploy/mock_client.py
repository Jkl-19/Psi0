...
DATASET_REPO_ID = "Remove_the_cap_turn_on_the_faucet_and_fill_the_bottle_with_water"
DATASET_ROOT = Path("./data/real") / DATASET_REPO_ID
...
class RTCWebSocketClient:
    ...
    def _send_thread(self):
        """Send dataset observations in the same payload format as the live robot client."""
        print(f"[MockClient] Starting dataset replay. Total frames: {len(dataset)}")

        # Wait for WebSocket connection, same as original.
        self._connected.wait()
        prev_tick = time.perf_counter()

        for i in range(len(dataset)):
            if not self._running or not running.is_set():
                break

            try:
                # 1. Get observation from dataset instead of camera/robot.
                batch = dataset[i]

                frame = batch["observation.images.egocentric"]

                # LeRobot gives image as torch Tensor: C,H,W float32.
                # Server expects image-like NumPy array: H,W,C uint8.
                ...
                # 2. Build payload exactly like original client.
                payload = {
                    "image": img_obs,
                    "state": state_obs,
                    "gt_action": None,
                ...
                # 3. Serialize exactly like original client.
                payload = convert_numpy_in_dict(payload, numpy_serialize)
                message = json.dumps(payload)

                # 4. Send exactly like original client.
                ...
                print(f"[MockClient] Send error: {e}")
                break
        ...
        print("\n=== MOCK DATASET REPLAY COMPLETE ===")
        print(f"Sent frames: {i + 1 if 'i' in locals() else 0}")
        self.stop()
